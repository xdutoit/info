...
function chargerCle(){
    let cle_dechiffr = prompt('Entrez une clé de déchiffrement (26 caractères)')
    if(cle_dechiffr && cle_dechiffr.length==26){
        for(let i=0;i<26;i++){
            let c = cle_dechiffr.charAt(i);
            if(c != '_'){
